...
def YOLO(input_layer, NUM_CLASS, model='yolov4', is_tiny=False):
    if is_tiny:
        ...
        elif model == 'yolov4_tiny':
            return YOLOv4_tiny_tiny(input_layer, NUM_CLASS)
        elif model == 'yolov3':
...
def YOLOv4_tiny_tiny(input_layer, NUM_CLASS):
    route_1, conv = backbone.cspdarknet53_tiny_tiny(input_layer)

    conv = common.convolutional(conv, (1, 1, 128, 128))

    conv_lobj_branch = common.convolutional(conv, (3, 3, 128, 128))
    conv_lbbox = common.convolutional(conv_lobj_branch, (1, 1, 128, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = common.convolutional(conv, (1, 1, 128, 128))
    conv = common.upsample(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = common.convolutional(conv, (3, 3, 128, 128))
    conv_mbbox = common.convolutional(conv_mobj_branch, (1, 1, 128, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
